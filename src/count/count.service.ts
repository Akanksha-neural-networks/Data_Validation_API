...
export class CountService {
  ...
  async countData(countDto: CountDto[]): Promise<any[]> {
    try {
      const result = [];
      await Promise.all(
        countDto.map(async (count: CountDto) => {
          ...
          switch (engine) {
            case 'postgres':
              ...
              try {
                const sqlQuery = `select count(*) as count from ${database}.${schema}.${table} `;
            ...
            case 'mysql':
              ...
              try {
                const sqlQuery = `select count(*) as count from ${database}.${table} `;
